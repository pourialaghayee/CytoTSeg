...
# WORK_DIR = "/mnt/lustre/home/claassen/clala950/DC-TSeg/Data/ICellCNN/"
# TEST_PKL = "/mnt/lustre/home/claassen/clala950/DC-TSeg/Data/ICellCNN//Test_data_cyto2_finetuned_clean_SINGLE.pkl"
# OUTPUT_PKL = "/mnt/lustre/home/claassen/clala950/DC-TSeg/Data/ICellCNN//Test_data_cyto2_finetuned_clean_SINGLE_unet_preds.pkl"
...
# class DoubleConv(nn.Module):
#     """Double convolution block used in both architectures"""
#     def __init__(self, in_ch: int, out_ch: int):
#         super().__init__()
#         self.block = nn.Sequential(
#             nn.Conv2d(in_ch, out_ch, 3, padding=1, bias=False),
...
# class SmallUNet(nn.Module):
#     """Fixed depth=2 U-Net (for older models)"""
#     def __init__(self, in_channels=1, out_channels=1, base=16):
#         super().__init__()
#         self.down1 = DoubleConv(in_channels, base)
#         self.pool1 = nn.MaxPool2d(2)
...
# class DynamicUNet(nn.Module):
#     """Variable depth U-Net (from comprehensive search pipeline)"""
#     def __init__(self, in_channels=1, out_channels=1, base=16, depth=2):
#         super().__init__()
#         self.depth = depth
...
# def normalize_image(img: np.ndarray, mode: str) -> np.ndarray:
#     """
#     Normalize image based on mode specified in config
#     Modes: 'pre_normalized', 'none', 'minmax', 'percentile', 'zscore'
#     """
#     img = np.asarray(img, dtype=np.float32)
...
#     # Fallback: if image is in 0-255 range, normalize to 0-1
#     if img.max() > 1.0:
#         img = img / 255.0
...
# def pad_images_to_common_size(images: List[np.ndarray], pad_mode: str = "reflect") -> List[np.ndarray]:
#     """Pad batch of images to same size for batching"""
#     Hmax = max(im.shape[0] for im in images)
#     Wmax = max(im.shape[1] for im in images)
#     out = []
...
#     # Normalize and pad to multiple of 4
#     normed = []
#     orig_sizes = []
...
#     # Pad to common size for batching
#     normed = pad_images_to_common_size(normed, pad_mode=pad_mode)
    
#     # Create batch tensor
#     batch = np.stack(normed, axis=0).astype(np.float32)  # [B,H,W]
#     x = torch.from_numpy(batch)[:, None, ...].to(DEVICE)  # [B,1,H,W]
...
# def load_config_and_model(work_dir: str):
#     """
#     Load model and config from training output directory.
#     Automatically detects architecture (DynamicUNet or SmallUNet).
#     """
#     # Find config file
#     cfg_path = os.path.join(work_dir, "config_unet_v2.json")
#     if not os.path.exists(cfg_path):
...
#     # Find checkpoint
#     ckpt = cfg.get("checkpoint_full_path", None)
#     if ckpt is None:
#         ckpt = cfg.get("checkpointfullpath", None)
#     if ckpt is None:
#         ckpt_name = cfg.get("checkpoint", "best_model.pth")
#         ckpt = os.path.join(work_dir, "checkpoints", ckpt_name)
...
#     # Extract threshold and preprocessing
#     threshold = float(cfg.get("threshold", 0.5))
#     prep = cfg.get("preprocessing", {})
#     norm_mode = prep.get("norm_mode", prep.get("normmode", "pre_normalized"))
...
# def extract_patient_images_and_gts(patient_data: Dict[str, Any]):
#     """
#     Extract images and ground truth from patient data.
#     Supports multiple key naming conventions.
#     """
#     # Try to find images key
#     images = None
#     for k in ("image", "images", "Image"):
#         if k in patient_data:
#             images = patient_data[k]
...
#     if images is None:
#         raise KeyError(
#             f"Could not find images key in patient_data. "
#             f"Available keys: {list(patient_data.keys())}"
#         )
    
#     # Try to find ground truth key (optional for test sets)
#     gts = None
#     for k in ("Ground_truth", "Groundtruths", "GuckThres", "masks", "mask", "Groundtruth"):
#         if k in patient_data:
#             gts = patient_data[k]
...
#         # Store predictions in patient data
#         pdata["Unet_preds"] = [p.astype(np.uint8) for p in preds]
...
#     overall = {
#         "n_patients": len(testdata),
#         "n_images_total": int(sum(v["n_images"] for v in per_patient.values())),
#         "threshold": float(threshold),
#         "norm_mode": norm_mode,
#         "batch_size": int(BATCH_SIZE),
#         "pad_mode": PAD_MODE,
...
#             (sum(v["n_images"] for v in per_patient.values()) / elapsed) 
...
#     # Save augmented test pickle with predictions
#     with open(OUTPUT_PKL, "wb") as f:
#         pickle.dump(testdata, f, protocol=pickle.HIGHEST_PROTOCOL)
